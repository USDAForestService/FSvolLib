...
For all floating point numbers use double precision for final results for calculations.
When preforming calculations: prefer using numbers in units that minimize the need for decimals in calculations
For example if the lowest precision for a calculation is in 1/100ths of an inch and the initial input for a
calculation is 18.11 inches convert the unit for the calculation to 1811 1/100ths before preforming calculations
then at the end of the calculation convert the final result back to inches by dividing 100 or whatever is appropriate.
...
When Naming class/struct fields prefer names that minimize ambiguity. As well minimize abbreviations and shortening of words
